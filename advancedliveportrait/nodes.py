...
def get_model_dir(m):
    base_path = os.path.abspath("advancedliveportrait/models")  
    return os.path.join(base_path, m)
...
class LP_Engine:
    ...
    def get_detect_model(self):
        if self.detect_model == None:
            ...
            self.detect_model = YOLO(model_path).to("cpu")
    ...
    def get_face_bboxes(self, image_rgb):
        ...
        pred = detect_model(image_rgb, conf=0.7, device="cpu")
...
class ExpressionEditor:
    ...
    def run(self, rotate_pitch, rotate_yaw, rotate_roll, blink, eyebrow, wink, pupil_x, pupil_y, aaa, eee, woo, smile,
            src_ratio, sample_ratio, sample_parts, crop_factor, src_image=None, sample_image=None, motion_link=None, add_exp=None, output_path=None):  
        ...
        new_rotate = get_rotation_matrix(s_info['pitch'] + es.r[0], s_info['yaw'] + es.r[1], s_info['roll'] + es.r[2])
